...
<div id="głowny">
    <?php
        ...
        if  ($b=$a->query("SELECT * FROM samochody;"))
        {
            ...
            {   
                print'<div class="wynik"><div class="zdjecie"></div><p>';
                print $c['marka'].' '.$c['model'].'<br> Stan: '.$c['stan'].' Cena: '.$c['cena'].'PLN</p>';
                print '<br><input type="button" id="wiecej'.$i.'" Value="wiecej"> <br><input type="button" id="mniej'.$i.'" Value="mniej"></div>';
                print '    <script>
                $(function(){
        
                    $("#wiecej'.$i.'").on("click", function(){
                        $(".wynik").eq('.$i.').css("height","340px")
                    });
                    $("#mniej'.$i.'").on("click", function(){
                        $(".wynik").eq('.$i.').css("height","170px")
                    });
                }) 
                </script>';
                $i = $i+1;
